...
import dubovskyi.avro.read.confluent.ScalaConfluentKafkaAvroDeserializer
 ...
 * This object provides the main point of integration between applications and this library.
  *
  * It is STRONGLY recommended that users of this library understand the concept behind [[dubovskyi.avro.schemas.policy.SchemaRetentionPolicies]].
 */
object AvroSerDe {
   ...
   * It requires the path to the Avro schema which defines the records to be read.
   */
  implicit class StreamDeserializer(dsReader: DataFrameReader) extends AvroDecoder {
